...
- [x] Badge 클래스 테스트 코드 구현
  - 금액에 따른 뱃지부여가 올바른지 확인한다.
    - 5천 원 이상 별, 1만 원 이상 트리, 2만 원 이상 산타
- [x] Discountor 클래스 테스트 코드 구현
  - 할인 금액 계산이 올바른지 확인한다.
    - 크리스마스 할인 금액 계산
    - 평일 할인 금액 계산
    - 주말 할인 금액 계산
    - 특별 할인 금액 계산
    - 증정 이벤트 금액 계산
- [x] ExpectedVisitDate 클래스 테스트 코드 구현
  - 방문 날짜가 올바른 경우 string 입력이 number로 변환
  - 방문 날짜가 올바르지 않은 경우 '[ERROR] 유효하지 않은 날짜입니다. 다시 입력해 주세요.'
- [x] Order 클래스 테스트 코드 구현
  - 주문 입력이 올바른 경우 string 입력이 Map로 변환
  - 주문 입력이 올바르지 않은 경우 '[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.'
